...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Workspace kind.** Children default to a throwaway `scratch` dir. To edit "
    "an existing repo, pass `workspace_kind=\"dir\"` + "
    "`workspace_path=\"/abs/repo/path\"` (or `\"worktree\"` for an isolated "
    "checkout). Valid `workspace_kind` values are ONLY `scratch`, `worktree`, "
    "`dir` — a board slug or project NAME is not a workspace kind, and "
    "passing one silently blocks every spawned worker.\n"
    "\n"
    "## Do NOT\n"
    "\n"
